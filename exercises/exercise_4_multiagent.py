...
def privacy_agent(state: State) -> dict:
    ...
    llm = get_llm()
    prompt = f"""Bạn là chuyên gia về bảo vệ dữ liệu cá nhân và luật privacy. Phân tích khía cạnh privacy trong câu hỏi:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR (phạt tối đa 4% doanh thu toàn cầu hoặc EUR 20M), CCPA (phạt $7,500/vi phạm cố ý),
data breach notification obligations, quyền của chủ thể dữ liệu, trách nhiệm của data controller/processor."""
...
def build_graph() -> StateGraph:
    ...
    # Define edges — check_routing is used as a routing function, not a node
